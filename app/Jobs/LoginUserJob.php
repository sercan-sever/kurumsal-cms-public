...
use App\Mail\SendLoginUserNotification;
...
class LoginUserJob implements ShouldQueue
{
    ...
    public function handle(EmailSetting $emailSetting, EmailService $emailService): void
    {
        ...
        if ($emailService->syncMailConfigWithDatabaseCheck(emailSetting: $emailSetting)) {
            ...
            Mail::to(users: $emailSetting?->notification_email, name: 'Localkod - Giriş Yapan Kullanıcı')->send(new SendLoginUserNotification(
                user: $this->user,
                ip: $this->ip,
                date: $this->date,

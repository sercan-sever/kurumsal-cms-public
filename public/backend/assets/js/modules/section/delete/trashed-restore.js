$(document).ready(function () {
    ...
    $(document).on("click", ".trashed-restore-section-delete-btn", async function (e) {
        e.preventDefault();
        ...
        try {
            // Axios ile istek at
            const response = await axios.post('/lk-admin/pages/sections/trashed/restore', { id: id });
            ...
            if (data.success) {
                ...
                let rowToDelete = $('#trashed-section-lists tbody tr').filter(function () {
                    return $(this).data('id') == section.id;
                });
                ...
                $(this).data('id', '');
                $('#trashed-restore-section-modal').modal('hide');
            }

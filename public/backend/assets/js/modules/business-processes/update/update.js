$(document).ready(function () {
    ...
    $(document).on("submit", "#update-form", async function (e) {
        ...
        try {
            ...
            if (data.success) {
                ...
                updateRowTable(businessProcesses, content, statusHtml);

                $('#update-business-processes-modal').modal('hide');
            }
    ...
    function updateRowTable(businessProcesses, content, statusHtml) {
        ...
        let row = datatable.row(`[data-id="${(businessProcesses.id ?? '')}"]`);
        ...
        rowData[1] = `${(content.header ?? '')}`; // Görseli güncelle

        rowData[2] = `${(content.title ?? '')}`;

        rowData[3] = `${(content.description ?? '')}`;

        rowData[4] = `${(statusHtml ?? '')}`;

        rowData[5] = `${(businessProcesses.sorting ?? '1')}`;

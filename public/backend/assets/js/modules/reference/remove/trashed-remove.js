$(document).ready(function () {
    ...
    $(document).on("click", ".trashed-remove-reference-delete-btn", async function (e) {
        e.preventDefault();
        ...
        try {
            // Axios ile istek at
            const response = await axios.post('/lk-admin/reference/trashed/remove', { id: id });

            const data = response.data;

            if (data.success) {
                let referenceId = data.reference;
                ...
                let rowToDelete = $('#trashed-reference-lists tbody tr').filter(function () {
                    return $(this).data('id') == referenceId;
                });
                ...
                $(this).data('id', '');
                $('#trashed-remove-reference-modal').modal('hide');
            }

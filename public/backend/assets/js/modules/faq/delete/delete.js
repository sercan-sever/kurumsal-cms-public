$(document).ready(function () {
    ...
    $(document).on("click", ".faq-delete-btn", async function (e) {
        e.preventDefault();
        ...
        if (empty(id)) {
            ...
            showSwal('info', 'S. S. S. Bulunamadı Sayfayı Yenileyerek Tekrar Deneyiniz !!!', 'center');
        ...
        try {
            // Axios ile istek at
            const response = await axios.post('/lk-admin/faq/delete', { id: id, deleted_description: deleted_description });
            ...
            if (data.success) {
                ...
                let rowToDelete = $('#faq-lists tbody tr').filter(function () {
                    return $(this).data('id') == faq.id;
                });
    ...
    function trashedTableAdd(faq, title) {
        let datatable = $('#trashed-faq-lists').DataTable();
        ...
        let description = `${(faq.deleted_description ?? '')}`;

        let sorting = `${(faq.sorting ?? '1')}`;

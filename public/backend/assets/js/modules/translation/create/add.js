$(document).ready(function () {
    $.ajaxSetup({
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        }
    });

    // Kapandığı Anda Form İçerisini Temizlemeye Yarar
    $(document).on('hidden.bs.modal', '#add-translation-modal', function () {
        resetAddForm();
    });

    $(document).on("submit", "#create-form", async function (e) {
        e.preventDefault();

        let url = $(this).data('action');
        waitShowAndButtonPassive();

        try {
            // Axios ile istek at
            const response = await axios.post(url, new FormData(this));

            const data = response.data;

            if (data.success) {
                let translationContent = data.translationContent;
                let isDeletable = data.isDeletable;

                tableAdd(translationContent, isDeletable);

                $('#add-translation-modal').modal('hide');

                resetAddForm();
            }

            waitHideAndButtonActive();

            showToast(data.message, data.success ? "#4fbe87" : "#DC3546");

        } catch (error) {
            const response = error.response;

            // Hata durumunu kontrol et
            if (response && response.status === 422) {
                $.each(response.data.errors, function (key, value) {
                    showToast(value[0], "#DC3546");
                });
            } else if (response && response.data.message) {
                showToast(response.data.message, "#DC3546");
            } else {
                showToast(error.message || 'Beklenmeyen Bir Hata Oluştu.', "#DC3546");
            }

            waitHideAndButtonActive();
        }
    });

    function tableAdd(translationContent, isDeletable) {
        let datatable = $('#translation-lists').DataTable();

        let buttons = `<button type="button" class="read-view-btn btn btn-icon btn-bg-light btn-light-primary btn-sm m-2">
                            <i class="fas fa-eye"></i>
                        </button>
                        <button type="button" class="edit-btn btn btn-icon btn-bg-light btn-light-info btn-sm m-2">
                            <i class="fa-solid fa-pencil"></i>
                        </button>
                        ${isDeletable ? `<button type="button" class="delete-btn btn btn-icon btn-bg-light btn-light-danger btn-sm m-2">
                            <i class="fa-solid fa-trash"></i>
                        </button>` : ''}`;

        let group = `${(translationContent.group ?? '')}`;

        let key = `${(translationContent.key ?? '')}`;

        let value = `${(translationContent.value ?? '')}`;

        // Yeni satırı ekle ve referans al
        let rowNode = datatable.row.add([
            buttons,
            group,
            key,
            value
        ]).draw(false).node(); // Tabloyu güncelle ve DOM düğümünü al

        // Eklenen satıra 'data-id' niteliği ekle
        $(rowNode).attr('data-id', translationContent.id ?? '');
        $(rowNode).find('td').eq(3).addClass('text-start');

        datatable.order([1, 'asc']).draw(false);
    }

    function resetAddForm() {
        $('#create-form').trigger('reset');
    }
});

$(document).ready(function () {
    ...
    $(document).on("submit", "#update-form", async function (e) {
        ...
        try {
            ...
            if (data.success) {
                ...
                updateRowTable(translationContent, isDeletable);
    ...
    function updateRowTable(translationContent, isDeletable) {
        ...
        let row = datatable.row(`[data-id="${(translationContent.id ?? '')}"]`);
        ...
        rowData[1] = `${(translationContent.group ?? '')}`;
        rowData[2] = `${(translationContent.key ?? '')}`;
        rowData[3] = `${(translationContent.value ?? '')}`;

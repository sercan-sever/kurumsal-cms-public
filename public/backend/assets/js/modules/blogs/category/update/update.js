$(document).ready(function () {
    ...
    $(document).on("submit", "#update-form", async function (e) {
        ...
        try {
            ...
            if (data.success) {
                ...
                updateRowTable(blogCategory, content, categoryCount, image, statusHtml);

                $('#update-blog-category-modal').modal('hide');
            }
    ...
    function updateRowTable(blogCategory, content, categoryCount, image, statusHtml) {
        ...
        let row = datatable.row(`[data-id="${(blogCategory.id ?? '')}"]`);
        ...
        rowData[3] = `${(categoryCount ?? '')}`;
        rowData[4] = `${(statusHtml ?? '')}`;
        rowData[5] = `${(blogCategory.sorting ?? '1')}`;

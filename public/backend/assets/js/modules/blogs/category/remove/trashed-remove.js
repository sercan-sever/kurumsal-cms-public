$(document).ready(function () {
    ...
    $(document).on("click", ".trashed-remove-blog-category-delete-btn", async function (e) {
        e.preventDefault();
        ...
        try {
            // Axios ile istek at
            const response = await axios.post('/lk-admin/blogs/category/trashed/remove', { id: id });

            const data = response.data;

            if (data.success) {
                let blogCategoryId = data.blogCategory;
                ...
                let rowToDelete = $('#trashed-blog-category-lists tbody tr').filter(function () {
                    return $(this).data('id') == blogCategoryId;
                });
                ...
                $(this).data('id', '');
                $('#trashed-remove-blog-category-modal').modal('hide');
            }

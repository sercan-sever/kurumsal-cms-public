$(document).ready(function () {
    ...
    $(document).on('hidden.bs.modal', '#view-blog-modal', function () {
        $('#view-blog-modal #view-blog-modal-scroll').empty();
    });
    ...
    $(document).on('click', '.read-view-btn', async function (e) {
        ...
        try {
            // Axios ile istek at
            const response = await axios.post('/lk-admin/blogs/read-view', { id: id });
            ...
            if (data.success) {
                ...
                let blog = data.blog;
                $('#view-blog-modal-scroll').html(blog ?? '');

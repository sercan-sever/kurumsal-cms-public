$(document).ready(function () {
    ...
    $(document).on("click", ".brand-delete-btn", async function (e) {
        e.preventDefault();
        ...
        if (empty(id)) {
            ...
            showSwal('info', 'Marka Bulunamadı Sayfayı Yenileyerek Tekrar Deneyiniz !!!', 'center');
        ...
        try {
            // Axios ile istek at
            const response = await axios.post('/lk-admin/brands/delete', { id: id, deleted_description: deleted_description });
            ...
            if (data.success) {
                ...
                let rowToDelete = $('#brand-lists tbody tr').filter(function () {
                ...
                if (!empty(rowToDelete)) {
                    ...
                    trashedTableAdd(brand, content, image);
                }
            }
    ...
    function trashedTableAdd(brand, content, image) {
        ...
        let description = `${(brand.deleted_description ?? '')}`;

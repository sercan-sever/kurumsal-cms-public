$(document).ready(function () {
    ...
    $(document).on("click", ".trashed-restore-brand-delete-btn", async function (e) {
        e.preventDefault();
        ...
        if (empty(id)) {
            ...
            showSwal('info', 'Silinen Marka Bulunamadı Sayfayı Yenileyerek Tekrar Deneyiniz !!!', 'center');
        ...
        try {
            // Axios ile istek at
            const response = await axios.post('/lk-admin/brands/trashed/restore', { id: id });
            ...
            if (data.success) {
                ...
                let datatable = $('#trashed-brand-lists').DataTable();

                // İlgili satırı direkt olarak seç
                let rowToDelete = $('#trashed-brand-lists tbody tr').filter(function () {
                    return $(this).data('id') == brand.id;
                });
                ...
                tableAdd(brand, content, image, statusHtml);

                $(this).data('id', '');
                $('#trashed-restore-brand-modal').modal('hide');
            }
    ...
    function tableAdd(brand, content, image, statusHtml) {

$(document).ready(function () {
    $.ajaxSetup({
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        }
    });

    $(document).on("submit", "#update-permission-form", async function (e) {
        e.preventDefault();

        let url = $(this).data('action');
        waitShowAndButtonPassive();

        try {
            // Axios ile istek at
            const response = await axios.post(url, new FormData(this));
            const data = response.data;

            if (data.success) {

                tableUpdate(data.user, data.html);

                resetUpdatePermissionForm();
            }

            waitHideAndButtonActive();

            showToast(data.message, data.success ? "#4fbe87" : "#DC3546");
        } catch (error) {
            const response = error.response;

            // Hata durumunu kontrol et
            if (response && response.status === 422) {
                $.each(response.data.errors, function (key, value) {
                    showToast(value[0], "#DC3546");
                });
            } else if (response && response.data.message) {
                showToast(response.data.message, "#DC3546");
            } else {
                showToast(error.message || 'Beklenmeyen Bir Hata Oluştu.', "#DC3546");
            }

            waitHideAndButtonActive();
        }
    });


    function resetUpdatePermissionForm() {
        $('#permission-user-management-modal').modal('hide');

        $('#update-permission-form').trigger('reset');
    }


    function tableUpdate(user, html) {
        let datatable = $('#user-management-lists').DataTable();

        // Mevcut satırı data-id ile bul
        let row = datatable.row(`[data-id="${(user.id ?? '')}"]`);

        if (row.length === 0) {
            showSwal('info', 'Güncellenicek Bir Satır Bulunamadı!!!', 'center');
            return;
        }

        // Satırdaki mevcut veriyi al
        let rowData = row.data();
        rowData[4] = `${(html ?? '')}`;

        // Güncellenen verileri satıra geri ekle
        row.data(rowData).draw();

        datatable.order([1, 'asc']).draw();
    }
});

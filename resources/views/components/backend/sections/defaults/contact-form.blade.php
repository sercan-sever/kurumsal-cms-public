@if (empty($disabled))
    <form id="update-form" class="form" data-action="{{ route('admin.pages.section.update.contact.form') }}">
        @csrf
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            @if ($section->allContent->isEmpty())
                                <x-backend.sections.defaults.contact-form.empty :language="$language" :loop="$loop" :pages="$pages" :section="$section" :disabled="$disabled" />
                            @else
                                <x-backend.sections.defaults.contact-form.full :language="$language" :loop="$loop" :pages="$pages" :section="$section" :disabled="$disabled" />
                            @endif

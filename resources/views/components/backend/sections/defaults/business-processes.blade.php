@if (empty($disabled))
    <form id="update-form" class="form" data-action="{{ route('admin.pages.section.update.business.processes') }}">
        @csrf
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            @if ($section->allContent->isEmpty())
                                <x-backend.sections.defaults.business-processes.empty :language="$language" :loop="$loop" :section="$section" :pages="$pages" :disabled="$disabled" />
                            @else
                                <x-backend.sections.defaults.business-processes.full :language="$language" :loop="$loop" :section="$section" :pages="$pages" :disabled="$disabled" />
                            @endif

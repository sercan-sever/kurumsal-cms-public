@if (empty($disabled))
    <form id="update-form" class="form" data-action="{{ route('admin.pages.section.update.mission.vision') }}">
        @csrf
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            @if ($section->allContent->isEmpty())
                                <x-backend.sections.defaults.mission-vision.empty :language="$language" :loop="$loop" :section="$section" :disabled="$disabled" />
                            @else
                                <x-backend.sections.defaults.mission-vision.full :language="$language" :loop="$loop" :section="$section" :disabled="$disabled" />
                            @endif

@if (empty($disabled))
    <form id="update-form" class="form" data-action="{{ route('admin.pages.section.update.brand') }}">
        @csrf
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            @if ($section->allContent->isEmpty())
                                <x-backend.sections.defaults.brand.empty :language="$language" :loop="$loop" :section="$section" :pages="$pages" :disabled="$disabled" />
                            @else
                                <x-backend.sections.defaults.brand.full :language="$language" :loop="$loop" :section="$section" :pages="$pages" :disabled="$disabled" />
                            @endif

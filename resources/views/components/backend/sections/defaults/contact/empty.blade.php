        ...
        <div class="col-md-6 mb-7">
            ...
            <select name="page" class="form-control form-select-solid" id="page" data-live-search="true" @disabled(!empty($disabled))>
                ...
                @foreach ($pages as $page)
                    <option value="{{ $page->id }}">{{ $page?->content?->title }}</option>
                @endforeach
            </select>
        </div>
    </div>
...
<div class="row">
    <div class="col-md-6 mb-7">
        ...
        <input type="text" name="{{ $language?->code }}[heading]" class="form-control form-control-solid" placeholder="Üst Başlık *" minlength="1" required @disabled(!empty($disabled)) />
    </div>
    <div class="col-md-6 mb-7">
        <label class="required fs-6 fw-semibold mb-2">Alt Başlık ( {{ $language?->getCodeUppercase() }} )</label>
        <input type="text" name="{{ $language?->code }}[sub_heading]" class="form-control form-control-solid" placeholder="Alt Başlık *" minlength="1" required @disabled(!empty($disabled)) />
    </div>
    <div class="col-md-6 mb-7">
        <label class="required fs-6 fw-semibold mb-2">Form Başlık ( {{ $language?->getCodeUppercase() }} )</label>
        <input type="text" name="{{ $language?->code }}[description]" class="form-control form-control-solid" placeholder="Form Başlık *" minlength="1" required @disabled(!empty($disabled)) />
    </div>
    <div class="col-md-6 mb-7">
        <label class="required fs-6 fw-semibold mb-2">Açık Rıza Metni ( {{ $language?->getCodeUppercase() }} )</label>
        <input type="text" name="{{ $language?->code }}[short_description]" class="form-control form-control-solid" placeholder="Açık Rıza Metni *" @disabled(!empty($disabled)) required />
    </div>
    <div class="col-md-6 mb-7">
        <label class="fs-6 fw-semibold mb-2">Buton Başlık ( {{ $language?->getCodeUppercase() }} )</label>
        <input type="text" name="{{ $language?->code }}[button_title]" class="form-control form-control-solid" placeholder="Buton Başlık..." @disabled(!empty($disabled)) />
    </div>
</div>

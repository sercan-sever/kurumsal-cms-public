...
@foreach ($section->allContent as $content)
    ...
    @if ($outerLoop->first)
        ...
        <div class="row">
            ...
            <div class="col-md-6 mb-7">
                <label class="fs-6 fw-semibold mb-2">Sayfa</label>
                <select name="page" class="form-control form-select-solid" id="page" data-live-search="true" @disabled(!empty($disabled))>
                    <option value="">- Boş -</option>
                    @foreach ($pages as $page)
                        <option value="{{ $page->id }}" @selected($page->id == $section?->page_id)>{{ $page?->content?->title }}</option>
                    @endforeach
                </select>
            </div>
        </div>
    ...
    <div class="row">
        <div class="col-md-6 mb-7">
            ...
            <input type="text" name="{{ $language?->code }}[heading]" value="{{ $content?->heading }}" class="form-control form-control-solid" placeholder="Üst Başlık *" minlength="1" required @disabled(!empty($disabled)) />
        </div>
        <div class="col-md-6 mb-7">
            <label class="required fs-6 fw-semibold mb-2">Alt Başlık ( {{ $language?->getCodeUppercase() }} )</label>
            <input type="text" name="{{ $language?->code }}[sub_heading]" value="{{ $content?->sub_heading }}" class="form-control form-control-solid" placeholder="Alt Başlık *" minlength="1" required @disabled(!empty($disabled)) />
        </div>
        <div class="col-md-6 mb-7">
            <label class="required fs-6 fw-semibold mb-2">Form Başlık ( {{ $language?->getCodeUppercase() }} )</label>
            <input type="text" name="{{ $language?->code }}[description]" value="{{ $content?->description }}" class="form-control form-control-solid" placeholder="Form Başlık *" minlength="1" required @disabled(!empty($disabled)) />
        </div>
        <div class="col-md-6 mb-7">
            <label class="required fs-6 fw-semibold mb-2">Açık Rıza Metni ( {{ $language?->getCodeUppercase() }} )</label>
            <input type="text" name="{{ $language?->code }}[short_description]" value="{{ $content?->short_description }}" class="form-control form-control-solid" placeholder="Açık Rıza Metni *" @disabled(!empty($disabled)) required />
        </div>
        <div class="col-md-6 mb-7">

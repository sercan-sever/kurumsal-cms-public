@if (empty($disabled))
    <form id="update-form" class="form" data-action="{{ route('admin.pages.section.update.about') }}">
        @csrf
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            @if ($section->allContent->isEmpty())
                                <x-backend.sections.defaults.about.empty :language="$language" :loop="$loop" :section="$section" :disabled="$disabled" />
                            @else
                                <x-backend.sections.defaults.about.full :language="$language" :loop="$loop" :section="$section" :disabled="$disabled" />
                            @endif

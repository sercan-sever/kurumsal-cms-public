...
@foreach ($section->allContent as $content)
    ...
    <div class="row mb-7">
        <div class="col-md-6 fv-row">
            <label class="required fs-6 fw-semibold mb-2">Misyon Başlık ( {{ $language?->getCodeUppercase() }} )</label>
            <input type="text" name="{{ $language?->code }}[heading]" value="{{ $content?->heading }}" class="form-control form-control-solid" placeholder="Misyon Başlık *" minlength="1" required @disabled(!empty($disabled)) />
        </div>
        <div class="col-md-6 fv-row">
            <label class="required fs-6 fw-semibold mb-2">Vizyon Başlık ( {{ $language?->getCodeUppercase() }} )</label>
            <input type="text" name="{{ $language?->code }}[sub_heading]" value="{{ $content?->sub_heading }}" class="form-control form-control-solid" placeholder="Vizyon Başlık *" minlength="1" required @disabled(!empty($disabled)) />
        </div>
    </div>

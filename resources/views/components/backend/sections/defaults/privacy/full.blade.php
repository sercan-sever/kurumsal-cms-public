...
@foreach ($section->allContent as $content)
    ...
    <div class="row">
        ...
        <div class="col-md-12 mb-7">
            ...
            <textarea name="{{ $language?->code }}[description]" id="update_description_{{ $language?->id }}" class="form-control form-control-solid cst-description" rows="6" placeholder="İçerik *" @disabled(!empty($disabled))>{!! $content?->description !!}</textarea>
        </div>
    </div>
...
<script>
    @foreach ($languages as $al)
        if (CKEDITOR.instances['update_description_{{ $al->id }}']) {
            CKEDITOR.instances['update_description_{{ $al->id }}'].destroy();
        }
        CKEDITOR.replace('update_description_{{ $al->id }}', {
            height: 500,
        });
    @endforeach
</script>

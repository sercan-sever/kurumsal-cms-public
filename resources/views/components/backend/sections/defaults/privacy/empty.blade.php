...
<div class="row">
    <div class="col-md-12 mb-7">
        ...
        <input type="text" name="{{ $language?->code }}[heading]" class="form-control form-control-solid" placeholder="Başlık *" required @disabled(!empty($disabled)) />
    </div>
    <div class="col-md-12 mb-7">
        <label class="required fs-6 fw-semibold mb-2">İçerik ( {{ $language?->getCodeUppercase() }} )</label>
        <textarea name="{{ $language?->code }}[description]" id="description_{{ $language?->id }}" class="form-control form-control-solid cst-description" rows="6" placeholder="İçerik *" @disabled(!empty($disabled))></textarea>
    </div>
</div>

@if (empty($disabled))
    <form id="update-form" class="form" data-action="{{ route('admin.pages.section.update.terms') }}">
        @csrf
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            @if ($section->allContent->isEmpty())
                                <x-backend.sections.defaults.terms.empty :language="$language" :loop="$loop" :section="$section" :disabled="$disabled" />
                            @else
                                <x-backend.sections.defaults.terms.full :language="$language" :loop="$loop" :section="$section" :disabled="$disabled" />
                            @endif

@if (empty($disabled))
    <form id="update-form" class="form" data-action="{{ route('admin.pages.section.update.contact') }}">
        @csrf
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            @if ($section->allContent->isEmpty())
                                <x-backend.sections.defaults.contact.empty :language="$language" :loop="$loop" :pages="$pages" :section="$section" :disabled="$disabled" />
                            @else
                                <x-backend.sections.defaults.contact.full :language="$language" :loop="$loop" :pages="$pages" :section="$section" :disabled="$disabled" />
                            @endif

...
<div class="fv-row mb-7">
    ...
    <textarea name="{{ $language?->code }}[description]" id="proccess_description_{{ $language?->id }}" class="form-control form-control-solid cst-description" rows="6" placeholder="Açıklama *" maxlength="1000" required @disabled(!empty($disabled))></textarea>
</div>
...
<script>
    @foreach ($languages as $al)
        $('#proccess_description_{{ $al->id }}').maxlength({
            alwaysShow: true,

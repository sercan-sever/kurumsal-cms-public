...
@if (empty($disabled))
    <form id="create-form" class="form" data-action="{{ route('admin.pages.section.create.dynamic') }}">
        @csrf
@endif
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div class="fv-row mb-7">
                                ...
                                <textarea name="{{ $language?->code }}[description]" id="create_description_{{ $language?->id }}" class="form-control form-control-solid cst-description ckeditor" rows="6" placeholder="İçerik *"></textarea>
                            </div>
...
<script>
    @foreach ($languages as $al)
        CKEDITOR.replace('create_description_{{ $al->id }}', {

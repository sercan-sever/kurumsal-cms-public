...
@if (empty($disabled))
    <form id="update-form" class="form" data-action="{{ route('admin.brand.update') }}">
        @csrf
                ...
                <div id="update-body-{{ $language->id }}" class="card-body p-0 tab-pane fade show {{ $loop->first ? 'active' : '' }}" role="tabpanel" aria-labelledby="update-tab-{{ $language->id }}">
                    <div class="card">
                        <div class="card-body">
                            ...
                            @foreach ($brand->allContent as $content)
                                ...
                                @if ($outerLoop->first)
                                    @if (!empty($disabled))
                                        <div class="accordion" id="kt_accordion_1">
                                                    ...
                                                    <div class="accordion-body">
                                                        <div class="col-md-12 fv-row">
                                                            <div class="row cst-mobil-checkbox">
                                                                ...
                                                                @endif
                                                                @if (!empty($brand?->deletedBy?->id))
                                                                    <div class="col-md-6 fv-row mb-4">
                                                                        <div class="mb-2 border-dashed border-gray-300 rounded">
                                                                            <div class="p-4 text-center">
                                                                                <div class="text-gray-800 text-hover-primary fs-6 fw-bold mb-2">Silen</div>
                                                                                <span class="text-gray-700 fw-semibold fs-6 mb-2">
                                                                                    {{ $brand?->deletedBy?->name }}
                                                                                </span>
                                                                                <span class="text-muted fw-semibold d-block fs-6 mb-2 mt-2">
                                                                                    {{ $brand?->deletedBy?->email }}
                                                                                </span>
                                                                                <span class="text-muted fw-semibold d-block fs-7 mb-2">
                                                                                    {!! $brand?->deletedBy?->getRoleHtml() !!}
                                                                                </span>
                                                                                <span class="text-muted fw-semibold d-block fs-7">
                                                                                    {{ $brand->getDeletedAt() }}
                                                                                </span>
                                            ...
                                            @if (!empty($brand?->deleted_description))
                                                <div class="accordion-item">
                                    ...
                                    @if (empty($disabled))
                                        <div class="fv-row mb-7">
                                            ...
                                            <div class="bgi-no-repeat bgi-position-center bgi-size-cover image-input image-input-empty image-input-outline image-input-placeholder m-5" data-kt-image-input="true" style="background-image: url({{ $brand->getImage() }})">
                                ...
                                <div class="fv-row mb-7">
                                    ...
                                    <textarea name="{{ $language?->code }}[description]" class="form-control form-control-solid" rows="4" placeholder="Açıklama" @disabled(!empty($disabled))>{{ $content?->description }}</textarea>
                                </div>
                                ...
                                @if ($outerLoop->first)
                                    <div class="row cst-mobil-checkbox">
                                        ...
                                        <div class="col-md-6 fv-row mb-4">
                                            <input type="number" name="sorting" value="{{ $brand?->sorting }}" class="form-control form-control-solid" id="update-sorting" placeholder="Sıralama" min="1" required @disabled(!empty($disabled)) />
                                        </div>

                                        <div class="col-md-6 fv-row mb-4 mt-2 px-7">
                                            <div class="form-check form-switch form-check-custom form-check-solid cst-status">
                                                <input type="checkbox" name="status" class="form-check-input" id="update-status" @checked($brand?->isActiveStatus()) @disabled(!empty($disabled))>
